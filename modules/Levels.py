...
class Level1():
    ...
    def setupWalls(self, wall_color):
        ...
        wall_positions = [
            [0, 0, 6.0, 600.0], [0, 0, 600, 6], [0, 600, 606, 6], [600, 0, 6, 606], [300, 0, 6, 66], [60, 60, 186, 6],
        ...
        for wall_position in wall_positions:  # 遍历所有的矩形
            wall = Wall(*wall_position, wall_color)  # 创建矩形并填充颜色 作为墙
    ...
    def setupPlayers(self, hero_image_path, ghost_images_path):
        ...
        self.hero_sprites.add(Player(287, 439, hero_image_path,[20,20]))
        for each in ghost_images_path:
            role_name = each.split('/')[-1].split('.')[0]  # 以“/”为分割符保留最后一段，最后一段以“.”为分割符保留第一段：即获取怪物的名字
            if role_name == 'Blinky':
        ...
        for row in range(19):
            for col in range(19):
                if (row == 7 or row == 8) and (col == 8 or col == 9 or col == 10):
                    ...
                    food = Food(30 * col + 32, 30 * row + 32, 6, 6, food_color,)
